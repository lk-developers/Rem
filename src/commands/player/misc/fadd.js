...
const handle = async (message, prefix) => {
	...
	const filterName =
		message.content.trim().split(`${prefix}fadd`)[1].trim() || false;
	...
	const result = player.applyFilter(filterName);
	...
	if (result.code == "alreadyActiveFilter") {
		message.reply("That filter is already applied!.");
		message.react("😡");
		return;
	}
};
